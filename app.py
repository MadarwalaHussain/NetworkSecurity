...
async def predict_route(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        preprocessor = load_object(file_path='final_models/preprocessing.pkl')
        model = load_object(file_path='final_models/model.pkl')
        network_model = NetworkModel(preprocessor=preprocessor, model=model)
        print(df.iloc[0])
        y_pred = network_model.predict(df)
        logging.info(f"Prediction made successfully: {y_pred}")
        print(y_pred)

        df['predicted_column'] = y_pred
        print(df['predicted_column'])
        df.to_csv('prediction_output/predicted_output.csv', index=False)
        logging.info("Prediction output saved to 'prediction_output/predicted_output.csv'.")
